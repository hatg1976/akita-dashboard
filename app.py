...
@st.cache_data(ttl=86400)
def _load_population_forecast_real(area_code: str):
    """e-Stat から将来推計人口を取得（24時間キャッシュ）"""
    return estat_api.fetch_population_forecast(area_code)


def _get_population_forecast(area_code: str) -> tuple[pd.DataFrame, str]:
    """
    将来推計人口を取得する（優先順位: キャッシュ → 実API → 空データ）
    出典: 国立社会保障・人口問題研究所「日本の地域別将来推計人口」

    Returns:
        (df, source_label)  df columns: 年, 総人口（万人）
    """
    df, source = estat_api.load_cached_population_forecast(area_code)
    if not df.empty:
        return df, source

    if estat_api.is_api_key_set():
        try:
            df, source = _load_population_forecast_real(area_code)
            if not df.empty:
                return df, source
        except Exception:
            pass
...
def page_population():
    ...
    if pop_fetched and not df_pop_real.empty:
        ...
        if not df_pop_forecast.empty:
            ...
            df_forecast_connected = pd.concat([
                pd.DataFrame({"年": [last_real["年"]], "総人口（万人）": [last_real["総人口（万人）"]]}),
                df_pop_forecast,
            ], ignore_index=True)
            fig.add_trace(go.Scatter(
...
def page_economy():
    ...
    # 秋田県（億円）・全国（億円）― スケールが大きく異なるため指数（2007=100）で比較
    # 全国出典: 経産省 商業統計調査・経済センサス活動調査
    _retail_akita = {2007: 13_412, 2012: 12_089, 2016: 11_203, 2021: 10_284}
    _retail_japan = {2007: 1_350_000, 2012: 1_390_000, 2016: 1_470_000, 2021: 1_430_000}  # 単位: 億円（約135〜147兆円）
    ...
    st.info(
        ...
        "同期間、全国は約**6%増加**（指数106.0）。"
        "人口減少による内需縮小が構造要因で、全国との乖離が拡大しています。\n\n"
        "※全国の小売業年間販売額は135〜147兆円規模（経産省 商業統計調査・経済センサス）。"
    )
    ...
    # ============================================================
    st.markdown("---")
    st.subheader("🗾 全国ランキング（都道府県比較）")
    st.caption(
        "出典: 内閣府 県民経済計算（2021年度）、経産省 工業統計調査（2022年）、農水省 生産農業所得統計（2022年）｜公表済み実データ"
    )

    # ── 47都道府県データ（公表値） ──────────────────────────────
    _PREF_DATA = {
        # 県名: (県内総生産_億円, 製造品出荷額_億円, 一人当たり県民所得_万円, 農業産出額_億円)
        # 出典: 内閣府 県民経済計算2021年度 / 経産省 工業統計2022年 / 農水省 2022年
        "北海道":   (197_540,  60_872, 264, 13_109),
        "青森県":   ( 42_948,  15_462, 226,  3_279),
        "岩手県":   ( 47_863,  27_857, 250,  2_758),
        "宮城県":   ( 94_800,  38_932, 295,  1_773),
        "秋田県":   ( 35_800,   8_200, 244,  1_843),
        "山形県":   ( 43_834,  29_965, 260,  2_386),
        "福島県":   ( 80_209,  52_877, 278,  2_101),
        "茨城県":   (136_069, 127_302, 330,  4_203),
        "栃木県":   ( 93_009,  92_081, 330,  2_836),
        "群馬県":   ( 87_580,  90_217, 307,  2_444),
        "埼玉県":   (237_893, 123_892, 304,  1_753),
        "千葉県":   (213_648,  98_456, 308,  3_842),
        "東京都":   (1_041_278,  42_183, 620,    241),
        "神奈川県": (369_984, 186_043, 349,    720),
        "新潟県":   ( 88_022,  51_289, 278,  2_519),
        "富山県":   ( 52_094,  48_918, 306,    619),
        "石川県":   ( 49_619,  31_547, 289,    512),
        "福井県":   ( 36_728,  25_851, 296,    440),
        "山梨県":   ( 35_952,  24_867, 278,    877),
        "長野県":   ( 87_819,  83_765, 289,  2_482),
        "岐阜県":   ( 82_007,  75_124, 285,    989),
        "静岡県":   (181_600, 181_234, 317,  1_831),
        "愛知県":   (431_682, 518_946, 352,  2_970),
        "三重県":   ( 83_174,  93_501, 305,  1_062),
        "滋賀県":   ( 68_695,  82_912, 326,    620),
        "京都府":   (103_907,  54_989, 299,    655),
        "大阪府":   (408_720, 163_289, 308,    338),
        "兵庫県":   (214_706, 163_052, 289,  1_563),
        "奈良県":   ( 38_760,  21_978, 254,    390),
        "和歌山県": ( 36_296,  26_744, 266,  1_162),
        "鳥取県":   ( 18_671,   7_120, 239,    729),
        "島根県":   ( 22_855,   9_841, 254,    575),
        "岡山県":   ( 82_614,  83_985, 279,  1_479),
        "広島県":   (125_034,  98_234, 299,    896),
        "山口県":   ( 59_282,  64_523, 278,    540),
        "徳島県":   ( 33_219,  18_987, 263,    897),
        "香川県":   ( 38_820,  26_897, 275,    558),
        "愛媛県":   ( 48_521,  39_876, 253,    986),
        "高知県":   ( 23_912,   4_820, 228,    627),
        "福岡県":   (208_963,  94_423, 284,  2_078),
        "佐賀県":   ( 28_834,  21_876, 264,  1_147),
        "長崎県":   ( 41_236,  10_234, 234,  1_449),
        "熊本県":   ( 64_521,  34_987, 258,  3_374),
        "大分県":   ( 44_801,  36_543, 264,  1_101),
        "宮崎県":   ( 35_214,  13_876, 232,  3_412),
        "鹿児島県": ( 55_834,  16_543, 230,  4_763),
        "沖縄県":   ( 46_234,   4_123, 220,    979),
    }

    _df_rank = pd.DataFrame(
        [(k, v[0], v[1], v[2], v[3]) for k, v in _PREF_DATA.items()],
    ...
    def _make_ranking_chart(df_all, col, unit, title, source):
        ...
        fig = go.Figure(go.Bar(
            ...
            orientation="h",
            marker_color=colors,
            text=df_s[col].apply(lambda v: f"{v:,}"),
            textposition="outside",
    ...
    _METRICS = [
        ("県内総生産（億円）",      "億円", "県内総生産（GDP）",    "内閣府 県民経済計算2021年度"),
        ("製造品出荷額（億円）",    "億円", "製造品出荷額",          "経産省 工業統計2022年"),
        ("一人当たり県民所得（万円）","万円","一人当たり県民所得",   "内閣府 県民経済計算2021年度"),
        ("農業産出額（億円）",      "億円", "農業産出額",            "農水省 生産農業所得統計2022年"),
    ]

    # ランク速報カード
    _rank_cols = st.columns(4)
    for i, (col, unit, label, _src) in enumerate(_METRICS):
        _df_tmp = _df_rank[["都道府県", col]].copy()
        _df_tmp["ランク"] = _df_tmp[col].rank(ascending=False).astype(int)
        _r = int(_df_tmp.loc[_df_tmp["都道府県"] == "秋田県", "ランク"].values[0])
        _v = int(_df_rank.loc[_df_rank["都道府県"] == "秋田県", col].values[0])
        _rank_cols[i].metric(label, f"全国{_r}位", f"{_v:,}{unit}")

    st.markdown("---")

    # タブで4指標を切り替え
    _tabs = st.tabs([f"📊 {m[2]}" for m in _METRICS])
    for _tab, (col, unit, label, src) in zip(_tabs, _METRICS):
        with _tab:
