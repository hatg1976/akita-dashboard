...
def page_overview():
    ...
    st.info(
        "🔗 **e-Stat API 連携** ｜ "
        "政府統計ポータル「e-Stat」と API 連携し、人口推計など公的統計データを取得しています（人口推計は年1回・10月公表）。\n\n"
        "🔄 **毎月1日 自動キャッシュ更新** ｜ "
        "GitHub Actions のスケジュール機能により、毎月1日に e-Stat から最新データを取得してサーバー内に保存します。"
        "アプリはこのキャッシュを読み込むため、表示が高速で API 利用制限の影響を受けません。\n\n"
        "⚙️ **CI/CDパイプライン** ｜ "
        "GitHub Actions → Hugging Face Spaces への自動デプロイを構築。診断士業務に DevOps の仕組みを応用しています。\n\n"
        "🌾 **秋田県特化** ｜ "
        "秋田県に実在する52中分類・16大分類の産業データ、東北4県比較、地域固有の課題・提言を収録しています。"
    )
    st.markdown("---")
