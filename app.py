...
from collector import (
    ...
    # 後継者問題
    get_successor_absence_trend,
    get_successor_by_industry,
    get_owner_age_distribution,
    get_closure_trend,
    get_succession_risk_by_city,
    # 労働市場
    get_job_ratio_trend,
    get_job_ratio_by_industry,
    get_minimum_wage_trend,
    get_labor_shortage_index,
    get_employment_by_form,
)
...
def page_successor():
    """後継者問題・廃業リスクマップ"""
    st.title("👴 後継者問題・廃業リスクマップ")
    st.markdown(
        "秋田県の事業承継危機を可視化します。"
        "**帝国データバンク「全国企業後継者不在率調査」・中小企業白書**をベースにした推計値です。"
    )
    st.markdown("---")

    # ── 上段：KPI ───────────────────────────────────────────────
    df_abs = get_successor_absence_trend()
    latest_akita = df_abs["秋田県（%）"].iloc[-1]
    latest_all   = df_abs["全国（%）"].iloc[-1]
    df_close = get_closure_trend()
    latest_close = df_close["廃業・休業件数"].iloc[-1]
    latest_hard  = df_close["うち後継者難"].iloc[-1]

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("後継者不在率（秋田）", f"{latest_akita}%",
              delta=f"全国比 +{latest_akita - latest_all:.1f}pt", delta_color="inverse")
    c2.metric("全国後継者不在率", f"{latest_all}%")
    c3.metric("廃業・休業件数（2023年）", f"{latest_close}件",
              delta="前年比 -30件", delta_color="off")
    c4.metric("うち後継者難による廃業", f"{latest_hard}件",
              delta=f"廃業全体の{latest_hard/latest_close*100:.0f}%", delta_color="inverse")
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.subheader("後継者不在率の推移")
        fig = go.Figure()
        fig.add_trace(go.Scatter(
            x=df_abs["年"], y=df_abs["秋田県（%）"],
            name="秋田県", mode="lines+markers",
            line=dict(color="#c0392b", width=3), marker=dict(size=8),
        ))
        fig.add_trace(go.Scatter(
            x=df_abs["年"], y=df_abs["全国（%）"],
            name="全国", mode="lines+markers",
            line=dict(color="#2980b9", width=2, dash="dash"), marker=dict(size=6),
        ))
        fig.update_layout(
            height=320,
            yaxis=dict(title="%", range=[45, 85]),
            xaxis_title="年",
            legend=dict(orientation="h", y=-0.28),
            margin=dict(b=60),
        )
        st.plotly_chart(fig, use_container_width=True)
        st.caption("出典：帝国データバンク「全国企業後継者不在率調査」をもとに推計")

    with col2:
        st.subheader("廃業・休業件数の推移")
        fig2 = go.Figure()
        fig2.add_trace(go.Bar(
            x=df_close["年"], y=df_close["廃業・休業件数"],
            name="廃業・休業件数", marker_color="#e74c3c",
        ))
        fig2.add_trace(go.Bar(
            x=df_close["年"], y=df_close["うち後継者難"],
            name="うち後継者難", marker_color="#c0392b",
        ))
        fig2.add_trace(go.Scatter(
            x=df_close["年"], y=df_close["開業件数"],
            name="開業件数", mode="lines+markers",
    ...
    with col3:
        st.subheader("業種別 後継者不在率")
        df_ind = get_successor_by_industry()
        df_ind_s = df_ind.sort_values("後継者不在率（%）", ascending=True)
        fig3 = px.bar(
            df_ind_s, x="後継者不在率（%）", y="業種",
            orientation="h", color="後継者不在率（%）",
        ...
        st.plotly_chart(fig3, use_container_width=True)
        st.caption("出典：帝国DB調査・秋田県推計値（2023年）")

    with col4:
        st.subheader("経営者年齢分布の変化")
        df_age = get_owner_age_distribution()
        fig4 = go.Figure()
        x_cats = df_age["年齢層"].tolist()
        fig4.add_trace(go.Bar(
            name="2013年", x=x_cats, y=df_age["2013年（%）"],
            marker_color="#85c1e9",
        ))
        fig4.add_trace(go.Bar(
            name="2023年", x=x_cats, y=df_age["2023年（%）"],
            marker_color="#c0392b",
        ))
        fig4.update_layout(
        ...
        st.plotly_chart(fig4, use_container_width=True)
        st.caption("出典：中小企業白書・秋田県推計値")

    st.markdown("---")

    # ── 市町村別廃業リスクマップ ─────────────────────────────────
    st.subheader("市町村別 廃業リスク概況（経営者70歳超・後継者不在の事業所比率推計）")
    df_city = get_succession_risk_by_city()
    df_city_s = df_city.sort_values("高リスク事業所比率（%）", ascending=False)

    fig5 = px.bar(
        df_city_s, x="市町村", y="高リスク事業所比率（%）",
        color="高リスク事業所比率（%）",
        color_continuous_scale=["#f9e79f", "#e74c3c", "#922b21"],
        text="高リスク事業所比率（%）",
    )
    fig5.update_traces(texttemplate="%{text:.1f}%", textposition="outside")
    fig5.update_layout(
        height=360, showlegend=False, coloraxis_showscale=False,
        yaxis=dict(title="%", range=[0, 50]),
        margin=dict(b=10, t=10),
    )
    st.plotly_chart(fig5, use_container_width=True)
    st.caption(
        "推計方法：経済センサス事業所データ × 経営者年齢分布 × 後継者不在率をもとに算出。"
        "実態調査値ではなく推計値。"
    )
...
- 農林水産・飲食・小売は不在率70%超
- 訪問時に「後継者はいますか？」を必ず確認
- 後継者不在でも**第三者承継（M&A・業種内譲渡）**の選択肢を提示

**【活用できる支援策】**
- 事業承継・引継ぎ補助金（50〜250万円）
- 秋田県事業引継ぎ支援センター（無料相談）
- 経営承継円滑化法（相続税猶予）
""")
    with col6:
...
> 「業界全体で後継者不在率が約70%と聞いています。
> 御組合の会員企業ではいかがでしょうか。
> 承継問題が組合の共通テーマになっているようであれば、
> 引継ぎセンターと連携した勉強会も一つの手だと思います。」

**【北秋田・仙北・小坂は特に高リスク】**
- 廃業後に地域の商圏・雇用が失われる前に早期対応が必要
""")


def page_labor_market():
    """労働市場ダッシュボード"""
    st.title("👷 労働市場ダッシュボード")
    st.markdown(
        "**厚生労働省「職業安定業務統計」・秋田労働局「秋田の雇用」**をベースにした"
        "秋田県の労働市場データです。"
    )
    st.markdown("---")

    # ── 上段：KPI ───────────────────────────────────────────────
    df_ratio = get_job_ratio_trend()
    latest_akita = df_ratio["秋田県"].iloc[-1]
    latest_all   = df_ratio["全国"].iloc[-1]
    df_wage = get_minimum_wage_trend()
    latest_wage  = df_wage["秋田県（円）"].iloc[-1]
    prev_wage    = df_wage["秋田県（円）"].iloc[-2]
    latest_tokyo = df_wage["全国最高（東京）"].iloc[-1]

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("有効求人倍率（秋田・2024年）", f"{latest_akita}倍",
              delta=f"全国比 +{latest_akita - latest_all:.2f}倍")
    c2.metric("有効求人倍率（全国・2024年）", f"{latest_all}倍")
    c3.metric("最低賃金（秋田・2024年）", f"{latest_wage}円",
              delta=f"+{latest_wage - prev_wage}円（前年比）")
    c4.metric("全国最高（東京）との差", f"▲{latest_tokyo - latest_wage}円",
              delta_color="inverse")
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.subheader("有効求人倍率の推移")
        fig = go.Figure()
        fig.add_trace(go.Scatter(
            x=df_ratio["年"], y=df_ratio["秋田県"],
            name="秋田県", mode="lines+markers",
            line=dict(color="#e74c3c", width=3), marker=dict(size=8),
        ))
        fig.add_trace(go.Scatter(
            x=df_ratio["年"], y=df_ratio["全国"],
            name="全国", mode="lines+markers",
            line=dict(color="#2980b9", width=2, dash="dash"), marker=dict(size=6),
        ))
        ...
        fig.update_layout(
            height=320,
            yaxis=dict(title="倍", range=[0.8, 2.0]),
            xaxis_title="年",
            legend=dict(orientation="h", y=-0.28),
            margin=dict(b=60),
        )
        st.plotly_chart(fig, use_container_width=True)
        st.caption("出典：厚生労働省「職業安定業務統計」")

    with col2:
        st.subheader("最低賃金の推移")
        fig2 = go.Figure()
        fig2.add_trace(go.Bar(
            x=df_wage["年度"], y=df_wage["秋田県（円）"],
            name="秋田県", marker_color="#e74c3c",
        ))
        fig2.add_trace(go.Scatter(
            x=df_wage["年度"], y=df_wage["全国加重平均（円）"],
            name="全国加重平均", mode="lines+markers",
            line=dict(color="#2980b9", width=2), marker=dict(size=6),
        ))
        fig2.add_trace(go.Scatter(
            x=df_wage["年度"], y=df_wage["全国最高（東京）"],
            name="全国最高（東京）", mode="lines",
        ...
        st.plotly_chart(fig2, use_container_width=True)
        st.caption("出典：厚生労働省「地域別最低賃金額改定状況」")
    ...
    col3, col4 = st.columns(2)
    with col3:
        st.subheader("業種別 有効求人倍率（2023年）")
        df_ind = get_job_ratio_by_industry()
        df_ind_s = df_ind.sort_values("有効求人倍率", ascending=True)
        fig3 = px.bar(
            df_ind_s, x="有効求人倍率", y="業種",
            orientation="h", color="有効求人倍率",
            color_continuous_scale=["#a9cce3", "#e74c3c", "#922b21"],
            text="有効求人倍率",
        )
        fig3.update_traces(texttemplate="%{text:.2f}倍", textposition="outside")
        fig3.add_vline(x=1.0, line_dash="dot", line_color="gray")
        fig3.update_layout(
        ...
        st.plotly_chart(fig3, use_container_width=True)
        st.caption("出典：秋田労働局「秋田の雇用」（2023年）")

    with col4:
        st.subheader("業種別 人手不足感（DI指数）")
        df_short = get_labor_shortage_index()
        df_short_s = df_short.sort_values("人手不足感DI", ascending=True)
        fig4 = px.bar(
            df_short_s, x="人手不足感DI", y="業種",
            orientation="h", color="人手不足感DI",
            color_continuous_scale=["#a9cce3", "#e74c3c", "#922b21"],
            text="人手不足感DI",
        )
        fig4.update_traces(texttemplate="%{text:.1f}pt", textposition="outside")
        fig4.update_layout(
            height=320, showlegend=False, coloraxis_showscale=False,
            xaxis=dict(range=[0, 80], title="DI（+：不足超過）"),
            margin=dict(r=60),
        )
        st.plotly_chart(fig4, use_container_width=True)
        st.caption("出典：日本商工会議所LOBO調査・秋田県推計（2023年）")

    st.markdown("---")

    # ── 雇用形態の推移 ────────────────────────────────────────────
    st.subheader("雇用形態別 就業者数の推移")
    df_form = get_employment_by_form()
    fig5 = make_subplots(specs=[[{"secondary_y": True}]])
    fig5.add_trace(go.Bar(
        x=df_form["年"], y=df_form["正規雇用（千人）"],
        name="正規雇用", marker_color="#2980b9",
    ), secondary_y=False)
    fig5.add_trace(go.Bar(
        x=df_form["年"], y=df_form["非正規雇用（千人）"],
        name="非正規雇用", marker_color="#e67e22",
    ), secondary_y=False)
    fig5.add_trace(go.Scatter(
        x=df_form["年"], y=df_form["非正規比率（%）"],
        name="非正規比率", mode="lines+markers",
        line=dict(color="#c0392b", width=2), marker=dict(size=7),
    ), secondary_y=True)
    fig5.update_layout(
    ...
    fig5.update_yaxes(title_text="千人", secondary_y=False)
    fig5.update_yaxes(title_text="非正規比率（%）", range=[30, 50], secondary_y=True)
    st.plotly_chart(fig5, use_container_width=True)
    st.caption("出典：総務省「就業構造基本調査」")
    ...
    with col5:
        st.markdown("""
**【人手不足への対応策（優先度順）】**
1. **省力化投資補助金**（中小企業省力化投資補助金）活用推進
2. 処遇改善による定着率向上（賃上げ促進税制）
3. 外国人材・シニア・女性の活用
4. 業務プロセスのDX化

**【最低賃金の課題】**
- 東京との格差 **▲{:,}円**（2024年）
- 毎年30〜50円引き上げが続いており、人件費計画の見直しが急務
""".format(latest_tokyo - latest_wage))
    with col6:
        st.markdown(f"""
**【理事長との面談での会話例】**

> 「建設・運輸では求人倍率が4〜5倍と深刻な状況です。
> 組合員の中で採用に困っている会員はいらっしゃいますか。
> 省力化補助金を活用して設備で補う方向と、
> 賃上げで人を確保する方向の両方を整理できますか。」

**【非正規比率 {df_form['非正規比率（%）'].iloc[-1]}%（2023年）】**
- 全国平均（36.9%）を上回る水準
- パート・アルバイト中心の業種は特に最低賃金の影響が大
""")
