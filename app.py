...
def page_policy():
    ...
    # ========== TAB6: 中小企業政策 ==========
    with tab6:
        st.subheader("中小企業政策 重点提言")
        st.markdown("秋田県内中小企業の持続的成長を支援するための政策提言です。")

        sme_policies = [
            {
                "テーマ": "生産性向上・DX推進",
                "課題": "秋田県内中小企業のDX化率は全国平均を下回り、特に製造業・食品加工業での遅れが顕著。人手不足対策としてのデジタル化が急務。",
                "提言": [
                    "県内中小企業向けIT導入補助金の採択枠を拡充（現行の1.5倍）し、食品製造業・商店街向け専用枠を新設する",
                    "中小企業デジタル化相談窓口を商工会・商工会議所に常設し、専門人材（IT支援担当者）を配置する",
                    "省力化投資補助金の活用促進のため、県内製造業を対象とした個別導入診断を無償実施する",
                ],
                "期待効果": "県内中小企業の労働生産性を5年間で15%向上、人手不足倒産リスクを軽減",
                "実施主体": "秋田県産業労働部・中小企業診断士協会・商工会議所",
                "優先度": "高",
            },
            {
                "テーマ": "販路開拓・輸出支援",
                "課題": "県内食品・農産物の輸出額は伸び悩み、国内市場縮小（人口減少）への対応として海外販路開拓が不可欠。",
                "提言": [
                    "秋田米・日本酒・水産物の台湾・東南アジア向け輸出支援を強化し、バイヤー招聘事業を年2回実施する",
                    "農商工等連携計画の認定取得を支援するため、県内専門家派遣制度を整備する",
                    "食品製造業者向けにHACCP・GAPなど国際規格取得費用の補助率を現行2/3から3/4に引き上げる",
                ],
                "期待効果": "県内食品・農産物輸出額を5年間で現状の2倍（約60億円）に拡大",
                "実施主体": "秋田県産業労働部・農林水産部・秋田県輸出促進協議会",
                "優先度": "高",
            },
            {
                "テーマ": "事業承継・後継者確保",
                "課題": "県内中小企業経営者の高齢化が進み、後継者不在による廃業が年間200件超と推定。地域産業の担い手喪失が深刻。",
                "提言": [
                    "事業承継・引継ぎ支援センターと商工会の連携を強化し、M&Aマッチング件数を年間30件以上に拡大する",
                    "若者起業家応援補助金の対象年齢を40歳未満から45歳未満に拡大し、Uターン起業者への加算を充実させる",
                    "親族内承継・従業員承継向けの税務・法務相談を無償提供し、中小企業診断士との協働体制を構築する",
                ],
                "期待効果": "廃業による雇用喪失を5年間で1,000人分抑制し、地域産業の連続性を確保",
                "実施主体": "秋田県中小企業支援センター・中小企業診断士協会・金融機関",
                "優先度": "高",
            },
            {
                "テーマ": "人材確保・定着支援",
                "課題": "若年層の県外流出が続き、中小企業の採用難が深刻。UIターン人材の活用と副業・兼業人材の積極受け入れが必要。",
                "提言": [
                    "副業・兼業人材活用促進補助金の予算を現行の1.5倍に増額し、DX・海外展開分野のプロ人材受け入れを優先支援する",
                    "UIターン就業支援補助金の申請手続きを簡素化し、マッチングから補助金受給まで90日以内に完結できる体制を整備する",
                    "県内中小企業の魅力発信を強化するため、企業訪問ツアーと採用説明会を首都圏・宮城県で年4回実施する",
                ],
                "期待効果": "UIターン就業者数を現状の1.5倍に増加、中小企業の欠員率を3ポイント改善",
                "実施主体": "秋田県産業労働部・秋田UIJターン支援センター・商工会議所",
                "優先度": "中",
            },
            {
                "テーマ": "資金調達・経営安定化",
                "課題": "物価高騰・エネルギーコスト上昇により中小企業の収益が圧迫。賃上げ対応と設備投資の両立が困難な企業が増加。",
                "提言": [
                    "賃上げ緊急支援事業の補助単価を引き上げ（正規1人あたり5万円→8万円）、令和9年度以降も継続支援を確保する",
                    "省エネ設備更新補助金の申請手続きをデジタル化し、採択から補助金交付までの期間を現行の半分（3か月以内）に短縮する",
                    "金融機関と連携した経営改善計画策定支援を強化し、中小企業診断士の伴走支援に対する費用補助を創設する",
                ],
                "期待効果": "補助金活用企業の3年後生存率を現状85%から90%以上に向上",
                "実施主体": "秋田県産業労働部・秋田銀行・北都銀行・中小企業診断士協会",
                "優先度": "中",
            },
        ]

        for i, policy in enumerate(sme_policies):
            prio_color = "#c6efce" if policy["優先度"] == "高" else "#ffeb9c"
            prio_text = "#276221" if policy["優先度"] == "高" else "#9c5700"
            with st.expander(
        ...
        sme_df = pd.DataFrame([
            {"テーマ": p["テーマ"], "優先度": p["優先度"],
             "提言数": len(p["提言"]),
             "X": i % 3, "Y": i // 3}
            for i, p in enumerate(sme_policies)
        ])
        themes_short = {
            "生産性向上・DX推進": "DX推進",
            "販路開拓・輸出支援": "販路開拓",
        ...
        sme_df["テーマ略"] = sme_df["テーマ"].map(themes_short)
        urgency = {"生産性向上・DX推進": 3, "販路開拓・輸出支援": 3, "事業承継・後継者確保": 4,
                   "人材確保・定着支援": 2, "資金調達・経営安定化": 2}
        impact = {"生産性向上・DX推進": 4, "販路開拓・輸出支援": 5, "事業承継・後継者確保": 4,
                  "人材確保・定着支援": 3, "資金調達・経営安定化": 3}
        sme_df["緊急度"] = sme_df["テーマ"].map(urgency)
        sme_df["経済効果"] = sme_df["テーマ"].map(impact)
        fig_sme = px.scatter(
            sme_df, x="緊急度", y="経済効果",
            text="テーマ略", color="優先度",
            size=[20]*len(sme_df),
            ...
            title="中小企業政策 優先度マトリクス",
        )
        fig_sme.update_traces(textposition="top center", marker=dict(opacity=0.7))
        fig_sme.update_layout(height=420, xaxis=dict(range=[1, 5]), yaxis=dict(range=[1, 6]))
